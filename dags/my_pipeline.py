...
access_key = Variable.get("azure_blob_access_key")
...
def transform_data(file_path):
    try:
        df = pd.read_csv(file_path)
        nan_df = df.isna()
        # Check null value, if null drop that row.
        if len(nan_df) > 0:
            df = df.dropna()
        else:
            df = df
...
def upload_blob(blob_name):
    ...
    file_path = local_dir+"/"+"transformed_telco_data.parquet"
...
with DAG(
    ...
    quality_check >> transform >> loading >> end
